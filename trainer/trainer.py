...
class Trainer(BaseTrainer):
        ...
        with tqdm(zip(*self.data_loader), desc=f"Training epoch {epoch}", total=total_iterations) as progress:
            for batch_idx, data_li in enumerate(progress):
                ...
                for dl_idx, data in enumerate(data_li):
                    ...
                    text_embeds_content, video_embeds_content, text_embeds_semantic, video_embeds_semantic = self.model(data)
                    output_content = sim_matrix(text_embeds_content, video_embeds_content)
                    loss_content = self.loss(output_content)
                    output_semantic = sim_matrix(text_embeds_semantic, video_embeds_semantic)
                    loss_semantic = self.loss(output_semantic)
                    loss = loss_content + loss_semantic
                    loss.backward()
                    self.optimizer.step()

                    detached_loss_content = loss_content.detach().item()
                    detached_loss_semantic = loss_semantic.detach().item()
                    ...
                    progress.set_postfix({"dl": dl_idx,"loss_content":detached_loss_content, "loss_semantic": detached_loss_semantic})
    ...
    def _valid_epoch(self, epoch):
        ...
        text_embed_arr_content = {x: [] for x in range(len(self.valid_data_loader))}
        vid_embed_arr_content = {x: [] for x in range(len(self.valid_data_loader))}
        text_embed_arr_semantic = {x: [] for x in range(len(self.valid_data_loader))}
        vid_embed_arr_semantic = {x: [] for x in range(len(self.valid_data_loader))}
        ...
        with torch.no_grad():
            ...
            for dl_idx, dl in enumerate(self.valid_data_loader):
                for data in tqdm(dl, desc=f"Validating dl{dl_idx}"):
                    ...
                    text_embed_arr_content[dl_idx].append(text_embeds_content.cpu())
                    text_embed_arr_semantic[dl_idx].append(text_embeds_semantic.cpu())
                    vid_embed_arr_content[dl_idx].append(video_embeds_content.cpu())
                    vid_embed_arr_semantic[dl_idx].append(video_embeds_semantic.cpu())
                    ...
                    sims_batch = 0.7 * sim_matrix(text_embeds_content, video_embeds_content) + 0.3 * sim_matrix(text_embeds_semantic, video_embeds_semantic)
        ...
        for dl_idx in range(len(self.valid_data_loader)):
            ...
            text_embeds_content = torch.cat(text_embed_arr_content[dl_idx])
            vid_embeds_content = torch.cat(vid_embed_arr_content[dl_idx])
            text_embeds_semantic = torch.cat(text_embed_arr_semantic[dl_idx])
            vid_embeds_semantic = torch.cat(vid_embed_arr_semantic[dl_idx])
            ...
            # sims = sim_matrix(text_embeds_content, vid_embeds_content).detach().cpu().numpy()
            sims = 0.7 * sim_matrix(text_embeds_content, vid_embeds_content).detach().cpu().numpy() + 0.3 * sim_matrix(text_embeds_semantic, vid_embeds_semantic).detach().cpu().numpy()
